...
class newFlag(flagrObject):
    ...
    def GET(self):
        ...
        if flag["userID"] != c.session.userID and not c.session.user["level"] == "GOD":
            ...
            c.session.pushAlert(("You didn't create the flag: %s, but we can allow you to make a copy of it!" % ps.baseBold(flag.title)),
                type="error",
                icon="fire",
                title="Oh no!")
            return
        ...
        elements = [
            ps.baseInput(type="text",
                ...
                placeholder="Title"),
            "<br /><br />",
            {"content": ps.baseTextarea(classes="span10",
                name="description",
                content=flag["description"]),
                "help": ps.baseSmall("Text can be formated with %s" % ps.baseAnchor("Daring Fireball's Markdown",
        ...
        editForm = ps.baseHeading("%s Editing flag: %s" % (
            ps.baseIcon(flag.icon),
            flag["title"]
        ), size=1)

        editForm += ps.baseHorizontalForm(action=c.baseURL+"/flag/%s/edit"% (
            self.members[0]),
           method="POST",
    ...
    def POST(self):
        ...
        if flag["userID"] != c.session.userID and not c.session.user["level"] == "GOD":
            self.head = ("303 SEE OTHER", [("location", str("/flag/#s/copy"%flag.id))])
            c.session.pushAlert(("You didn't create the flag: %s by %s, but we can make a copy of it for you..." % (ps.baseBold(flag.title), flag.author)),
                type="error",
                icon="fire",
                title="Oh no!")
            return
        ...
        c.session.pushAlert(("Your flag: %s has been updated!" % ps.baseBold(flag.title)),
            type="success",
            icon="ok",
            title="YAY!")
...
class newFlagChoice(flagrObject):
    ...
    def GET(self):
        ...
        for flag in types:
                flag["description"] += "<br /><br />"
                flag["description"] += ps.baseAButton(ps.baseIcon("flag")+" I want this one!",
                    link=c.baseURL+"/flags/new/"+flag["type"],
                    classes="btn-info")
                newSelect.append(ps.baseTextThumbnail(label=flag["title"],
                    caption=flag["description"],
                    width=5))

        self.view.body = ps.baseHeading("%s Take your preference..." % ps.baseIcon("flag"),
            size=1)
...
class newFlag(flagrObject):
    ...
    def GET(self):
        ...
        editForm = ps.baseHeading("%s Make a new %s flag" % (
            ps.baseIcon(flag.icon),
            flag["flagType"]
        ), size=1)
    ...
    def POST(self):
        ...
        fc.updateSearch()
        self.head = ("303 SEE OTHER", [("location", str("/flag/%s"%flag.id))])
        c.session.pushAlert(("You created the flag: %s!" % ps.baseBold(flag.title)),
            type="success",
            icon="ok",
            title="YAY!")
...
class deleteFlag(flagrObject):
    ...
    def GET(self):
        ...
        if flag["userID"] != c.session.userID and not c.session.user["level"] == "GOD":
            ...
            c.session.pushAlert("You didn't create the flag: %s, so we can't allow you to delete it. Sorry!",
                type="error",
                icon="fire",
                title="Oh no!")
            return
        ...
        confirm = ps.baseHeading("Are you sure?",
            size=1,
            classes="text-error")
        confirm += ps.baseHeading("You are about to delete your flag: %s"% flag.title,
            classes="text-error",
            size=3)
        confirm += ps.baseParagraph("Pressing confirm will delete this flag forever and you will not be able to recover it. Are you sure you would like to continue?",
        ...
        confirmForm = ps.baseBasicForm(action=c.baseURL+"/flag/%s/delete"%flag.id,
            ...
            fields=[
                ps.baseButtonGroup([
                    ps.baseSubmit("Yes, Delete it.",
                        classes="btn-danger"),
                    ps.baseAButton("NO, Do Not Delete!",
    ...
    def POST(self):
        ...
        if flag["userID"] != c.session.userID and not c.session.user["level"] == "GOD":
            self.head = ("303 SEE OTHER", [("location", str("/flags"))])
            c.session.pushAlert(("You didn't create the flag: %s, so we can't allow you to delete it. Sorry!" % ps.baseBold(flag.title)),
                type="error",
                icon="fire",
                title="Oh snap!")
            return
        ...
        c.session.pushAlert(("The flag %s was deleted" % ps.baseBold(flag.title)),
            type="error")
...
class copyFlag(flagrObject):
    ...
    def GET(self):
        ...
        editForm = ps.baseHeading("%s Copying flag: %s" % (ps.baseIcon(flag.icon),
            flag["title"]),
            size=1)
    ...
    def POST(self):
        ...
        fc.updateSearch()
        self.head = ("303 SEE OTHER", [("location", str("/flag/%s"%flag.id))])
        c.session.pushAlert("You made a copy of flag: %s called %s!" % (
                ps.baseBold(self.members["oldTitle"]),
                ps.baseBold(flag["title"])
            ),
            type="success",
            icon="ok",
            title="YAY!")
...
class viewFlag(flagrObject):
    def GET(self):
            ...
            if not flag.visibility and flag.userID != c.session.userID and not c.session.user["level"] == "GOD":
                ...
                content += ps.baseHeading("%s %s" % (ps.baseIcon(flag.icon),
                    flag.title),
                    size=1, classes="")
                ...
                content += ps.baseRow(
                    ps.baseColumn(
                        ps.baseWell(
                            ps.baseColumn(ps.baseBold("Author: ",
                                classes="muted")) +
                            ps.baseColumn(author) +
                            ps.baseColumn(ps.baseBold("When: ",
                                classes="muted")) +
                ...
                for field in flag.fields:
                    ...
                    if name not in ["title",
                        ...
                        if name in ["url"]:
                            ...
                            value = flag[name]
                        content += ps.baseRow(ps.baseColumn("%s: %s" %(ps.baseBold(name.lower().title(),
                            classes="muted"),
                            value)))
                ...
                for label in flag["labels"]:
                    labelLinks += ps.baseAnchor(ps.baseLabel(label,
                        classes="label-info"),
                        link=c.baseURL+"/label/%s"%label) + " "

                content += ps.baseRow([
                    ps.baseColumn(ps.baseBold("Labels: ",
                        classes="muted"),
                        width=1),
