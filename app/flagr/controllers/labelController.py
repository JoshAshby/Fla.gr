...
class labelView(flagrObject):
    def GET(self):
        ...
        pageHead = ps.baseColumn(
            ps.baseHeading("%s Flags in label: %s" % (ps.baseIcon("tag"),
                    stack),
                size=2))
        ...
        pageHead += ps.baseRow(
            [
                ps.baseColumn(
                    ps.baseBold("%s Stacked labels:"%ps.baseIcon("tags"),
                        classes="muted")),
                ps.baseColumn(labelsUnder)
            ]) + "<br />"
        ...
        self.view.body = pageHead + ps.baseRow(
