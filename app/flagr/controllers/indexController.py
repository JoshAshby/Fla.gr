...
class index(publicObject):
    ...
    def GET(self):
        ...
        if carousel:
            carouselList = [ ps.baseColumn(item["content"],
                offset=1,
                width=6) for item in carousel if item["visibility"] ]
        ...
        if posts:
            for post in posts:
                if post["visibility"]:
                    if c.session.loggedIn and c.session.user["level"] in ["GOD", "admin"]:
                        edit = ps.baseSplitDropdown(btn=ps.baseAButton(ps.baseIcon("zoom-in"),
                            ...
                            dropdown=ps.baseMenu(name="postDropdown",
                                items=[{"name": "%s View as admin" % ps.baseIcon("cogs"),
                                    "link": c.baseURL+"/admin/post/%s/view"%post.id},
                                    {"name": "%s Edit" % ps.baseIcon("edit"),
                                        "link": c.baseURL+"/admin/post/%s/edit"%post.id},
                                    {"name": ps.baseBold("%s Delete"%ps.baseIcon("trash"),
                                        classes="text-error"),
                    ...
                    content += ps.baseRow([
                        ps.baseColumn(
                            ps.baseParagraph(post["post"]
                                [:250]+ps.baseAnchor("...Expand...",
                                    link=c.baseURL+"/post/"+post.id))),
                        ])
                    content += "<hr>"
