...
''')
        if VFFSL(SL,"user.loggedIn",True): # generated from line 52, col 29
            write(u'''                                <li ''')
            if VFFSL(SL,"page",True) in ('labels', 'settings', 'flags'): # generated from line 53, col 37
...
''')
        if VFFSL(SL,"user.alerts",True): # generated from line 108, col 21
            write(u'''                        ''')
            _v = VFFSL(SL,"user.alerts",True) # u'$user.alerts' on line 109, col 25
            if _v is not None: write(_filter(_v, rawExpr=u'$user.alerts')) # from line 109, col 25.
            write(u'''
''')
        write(u'''                </div>
''')
